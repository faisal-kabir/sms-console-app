...
The reference project `desset_wallet` uses BLoC (`flutter_bloc`) as its state management pattern, alongside dependency injection via `get_it`.
...
### 2. Riverpod
* **Why rejected**: Riverpod is a powerful modern state management solution, but it is not used in the reference project `desset_wallet`. Adding a new state management package would violate the primary constraint: "Follow the existing conventions of my project... state management, folder structure, lint rules, and theming approach."
...
1. **Repository Alignment**: Directly aligns with the conventions and setup found in `desset_wallet`.
